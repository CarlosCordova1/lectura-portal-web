...
/*  __________________________________________________________________________________________________  */
	...
	FUNCTION buscar_usuario_token (Vusuario in varchar2, passw in varchar2) RETURN VARCHAR2 IS
	     PRAGMA AUTONOMOUS_TRANSACTION;
      identificador    LCT_USER.ID_USER%TYPE;
     mail    LCT_USER.EMAIL%TYPE;
      nameusuario    LCT_USER.NOMBRE%TYPE;
...
/*  __________________________________________________________________________________________________  */
    ...
    /*  __________________________________________________________________________________________________  */
FUNCTION get_datadefault (  oratkn varchar2) RETURN clob IS
	cadena			clob;
	temp			json;
	temp2			json;
  temp3			json;
    ...
    --     servicios_ref_operativo number(10);
     ...
     ----------------------------------------------------------- 
      ----------------------------------------------------------- 
          DECLARE
          conanomalia number(10):=0;
               CURSOR C_user is SELECT * FROM LCT_USER where ESTATUS=1;
                    BEGIN              
                 FOR ind IN C_user LOOP
                 conanomalia :=conanomalia+1;
                        temp :=json();
                            ...
                            temp.put('nombre',ind.NOMBRE||' '||ind.APATERNO||' '||ind.AMATERNO);
                             ...
                             temp.put('token',buscar_usuario_token(ind.USUARIO,ind.PASS) );
                             temp.put('userlogin',ind.USUARIO );
                              temp.put('passwlogin',ind.PASS );
