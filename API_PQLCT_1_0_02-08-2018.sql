  ...
  cadena			clob;
    ...
    DECLARE
      ...
      --iduser :=json_ext.get_number(jsondescifrar,'iduser');
       ...
       ORDER BY IDtMOB DESC;
       */
       SELECT  IDTLEC, RNG1 SECUENCIA, NUMCPR MEDIDOR,fchlec,FCHVER, LECTURA, lecturaver,idtanm1,idtanm2, edoatl FROM app.lct_lecturas where idtodt =vODT and edoatl='CAP'
       order by RNG1 asc;
            begin 
  ...
  PRAGMA AUTONOMOUS_TRANSACTION;
    ...
    datos2 json;
     ...
     -- IF TOT!=0 THEN        
          ...
          update app.lct_detrtl set ORD=json_ext.get_number(json(medidor.get(i)),'Nuevoorden') where IDTRTL=vruta and IDTPNTLVR=json_ext.get_number(json(medidor.get(i)),'IdMedidor');
